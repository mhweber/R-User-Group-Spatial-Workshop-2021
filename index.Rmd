...
author: "Marc Weber"
date: "`r Sys.Date()`"
description: This is a draft of material for the R Spatial Workshop at the EPA R User Group Meeting in September 2021.
documentclass: book
...
## Workshop agenda
  ...
  + 1:00pm - 1:30pm EDT: [Intro and quick intro to R spatial objects and methods](https://mhweber.github.io/R-User-Group-Spatial-Workshop-2021/index.html)
  + 1:30pm - 1:55pm EDT: [Getting spatial data into R](https://mhweber.github.io/R-User-Group-Spatial-Workshop-2021/reading-in-spatial-data.html)
  + 1:55: 2:00 Break
  + 2:00pm - 2:30pm EDT: [Understanding vector and raster data in R and some example workflows](https://mhweber.github.io/R-User-Group-Spatial-Workshop-2021/index.html#spatial-data-in-r)
  + 2:30: 2:45 Break
  + 2:45pm - 3:00pm EDT: [Understanding coordinate reference systems (CRS)](https://mhweber.github.io/R-User-Group-Spatial-Workshop-2021/coordinate-reference-systems.html)
  + 3:00pm - 3:25pm EDT: [Vector data with sf](https://mhweber.github.io/R-User-Group-Spatial-Workshop-2021/vector-data-with-sf.html)
  + 3:25pm - 3:33pm Break
  + 3:30pm - 4:30pm EDT: [Raster data](https://mhweber.github.io/R-User-Group-Spatial-Workshop-2021/raster-data.html)
  + 4:30pm - 4:30pm EDT: [Geoprocessing](https://mhweber.github.io/R-User-Group-Spatial-Workshop-2021/geoprocessing.html)
  + 4:30pm - 5:00pm EDT: [Learning ways to map and visualize spatial data](https://mhweber.github.io/R-User-Group-Spatial-Workshop-2021/visualizing-spatial-data.html)
  
  
  + Along the way we'll look at: 
    
    - Select `tidyverse` packages and syntax, i.e. `ggplot2`, `dplyr`, `readr`, `tidyr`, the pipe operator %>% 
    - Working with vector and raster data in R
  + If this is all new, don't sweat it - google things, we'll answer questions as we go
  + There is no expectation of experience with spatial analysis in R - if you already have some experience, you are sure to pick up new tricks - if you don't, we'll cover the basics
...
```{r packages, message=FALSE, warning=FALSE, error=FALSE, eval=FALSE}
...
# install.packages("tmaptools")  # (already included under tmap)
install.packages("dplyr")
install.packages("palmerpenguins")
install.packages("tigris")
install.packages("tibble")
# install.packages("rgdal")      # (installs with tigris and sf)
install.packages("mapview")
# install.packages("leaflet")    # (installs with tmap and mapview)
install.packages("terra")
# need to install spDataLarge from Jacob Nowasad's repo
...
## Downloading content via Github
  + From [the workshop GitHub repository](https://github.com/mhweber/R-User-Group-Spatial-Workshop-2021) clone the repository by [copying the clone link](https://github.com/mhweber/R-User-Group-Spatial-Workshop-2021.git)
  + If you're not familiar with git and GitHub, simply choose to [download the zip file](https://github.com/mhweber/R-User-Group-Spatial-Workshop-2021/archive/refs/heads/master.zip) to have all the material locally

## Using RMarkdown and RStudio
  + R Markdown allows for reproducible reporting from R
  ...
  + We will assume everyone in the workshop is familiar with using RStudio
...
- Works with contributed packages - currently 18,188 - extensibility
...
options(scipen=3)
stats <- cran_downloads(from = "2018-01-01", to = "2021-09-01",packages = c("sp", "sf", "raster", "tmap", "mapview", "leaflet","ggmap", "stars", "terra"))
...
`sf` swamps everything! limit the y-axis
```{r spatial packages2, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}
ggplot(monthly_stats, aes(x=month, y=amount, group = package, colour = package)) + geom_line(size=1.5) + ylim(0, 250000) + scale_colour_brewer(palette="Set1") + ggtitle("R Monthly Spatial Package \n Downloads since 2018")
```
...
`sp` was the core vector spatial data package in R for a number of years, and while still used and while many packages still depend on the package, we will focus entirely `sf` for working with vector data.
...
- `terra` - The replacement package for `raster` which has been the primary spatial package for working with raster data in R
...
2. It is looking up a named feature in OpenStreetMap and returning the coordinates (bonus - we'll delve into more in next section - what coordinate reference system are coordinates in and how do you find out?)
...
```{r tmap example, fig.width=8, results='hide', message=FALSE, error=FALSE, warning=FALSE, results='hide'}
library(tigris)
...
### Interactive mapping
```{r mapview example, fig.width=8,message=FALSE, error=FALSE, warning=FALSE, results='hide'}
library(mapview)
